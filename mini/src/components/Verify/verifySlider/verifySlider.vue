<template>
  <view style="position: relative; touch-action: none; touch-action: pan-y">
    <view
      ...
      :style="{ height: `${parseInt(imgSize.height) + space}px` }"
    >
      <view
...
// 请求背景图片和验证图片
import { captchaCheckApi, captchaGetApi } from '@/api/common';
...
const emit = defineEmits(['update:clickShow', 'success']);
...
const getPictrue = async () => {
  ...
  try {
    const res = (await captchaGetApi(data)) as any;
...
const init = () => {
  ...
  getPictrue();
  nextTick(() => {
    instance?.proxy?.$parent?.$emit('ready', instance.proxy);
  });
};
...
const move = (e: any) => {
  nextTick(() => {
    ...
    barArea.value = query.select('.verify-bar-area');
        ...
        if (status.value && isEnd.value === false) {
          ...
          if (props.type !== '1') {
            ...
            if (
              move_block_left
              >= barArea_offsetWidth
              - Number.parseInt(Number.parseInt(props.blockSize.width) / 2)
              - 2
            ) {
              move_block_left
                = barArea_offsetWidth
          ...
          if (move_block_left <= 0) {
            move_block_left = Number.parseInt(
              Number.parseInt(props.blockSize.width) / 2,
            );
          }
...
const end = async () => {
  ...
  if (status.value && isEnd.value === false) {
    if (props.type !== '1') {
      ...
      moveLeftDistance
        = (moveLeftDistance * 310) / Number.parseInt(props.imgSize.width);
      ...
      try {
        console.log(data, captchaVerification);
        const res = await captchaCheckApi(data);
        ...
        setTimeout(() => {
          tipWords.value = '';
          emit('success', { captchaVerification: res });
        }, 1000);
      }
      catch (_e: any) {
        console.log(_e);
        ...
        instance?.proxy?.$parent?.$emit('error', instance.proxy);
        tipWords.value = '验证失败';
        setTimeout(() => {
          tipWords.value = '';
        }, 1000);
